...
class ChatKotlinApplicationTests {
    ...
    fun setUp() {
        ...
        val savedMessages = messageRepository.saveAll(
            listOf(
                Message("*testMessage*", ContentType.PLAIN, twoSecondBeforeNow, "test", USER_AVATAR_IMAGE_LINK),
                Message("**testMessage2**", ContentType.MARKDOWN, secondBeforeNow, "test1", USER_AVATAR_IMAGE_LINK),
                Message("`testMessage3`", ContentType.MARKDOWN, now, "test2", USER_AVATAR_IMAGE_LINK)
            )
        )
        ...
        if (!withLastMessageId) {
            assertThat(messages?.map { it.prepareForTesting() })
                ...
                .isEqualTo(
                    MessageVM(
                        "*testMessage*",
                        UserVM("test", URL(USER_AVATAR_IMAGE_LINK)),
        ...
        assertThat(messages?.map { it.prepareForTesting() })
            .containsSubsequence(
                MessageVM(
                    ...
                    UserVM("test1", URL(USER_AVATAR_IMAGE_LINK)),
                    now.minusSeconds(1).truncatedTo(MILLIS)
                ),
                MessageVM( "<body><p><code>testMessage3</code></p></body>", UserVM("test2", URL(USER_AVATAR_IMAGE_LINK)), now.truncatedTo(MILLIS))
            )
    }
    ...
    fun `test that messages posted to the API is stored`() {
        client.postForEntity(
            URI("/api/v1/messages"),
            MessageVM("`HelloWorld`", UserVM("test", URL(USER_AVATAR_IMAGE_LINK)), now.plusSeconds(1)),
...
const val USER_AVATAR_IMAGE_LINK = "http://test.com"
